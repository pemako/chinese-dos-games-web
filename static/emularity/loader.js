...
(function (Promise) {
  ...
  function IALoader(canvas, game, callbacks, scale) {
    ...
    if (!callbacks || typeof callbacks !== "object") {
      callbacks = { before_emulator: updateLogo, before_run: callbacks };
    } else {
            ...
            function (data) {
              ...
              if (module && module.indexOf("dosbox") === 0) {
                ...
                throw new Error(
                  "Unknown module type " +
                    module +
                    "; cannot configure the emulator.",
                );
              }

              var wantsWASM =
                modulecfg.wasm_filename && "WebAssembly" in window;
              var nr = modulecfg["native_resolution"];
              config_args = [
                cfgr.emulatorJS(
                  get_js_url(
                    wantsWASM
                      ? modulecfg.wasmjs_filename
                      : modulecfg.js_filename,
                  ),
                ),
                cfgr.emulatorWASM(
                  wantsWASM && get_js_url(modulecfg.wasm_filename),
                ),
              ...
              if (/archive\.org$/.test(document.location.hostname)) {
                cfgr.muted(
                  !(typeof $ !== "undefined" && $.cookie && $.cookie("unmute")),
                );
              }
              ...
              if (module && module.indexOf("dosbox") === 0) {
                config_args.push(
                  cfgr.startExe(
                    metadata.getElementsByTagName("emulator_start").item(0)
                      .textContent,
                  ),
                );
              } else if (module && module.indexOf("vice") === 0) {
                let emulator_start_item = metadata
                  .getElementsByTagName("emulator_start")
                  .item(0);
                let vice_fliplist = [
                  metadata
                    .getElementsByTagName("vice_drive_8_fliplist")
                    .item(0),
                  metadata
                    .getElementsByTagName("vice_drive_9_fliplist")
                    .item(0),
                  metadata
                    .getElementsByTagName("vice_drive_10_fliplist")
                    .item(0),
                  metadata
                    .getElementsByTagName("vice_drive_11_fliplist")
                    .item(0),
                ];
                if (emulator_start_item) {
                  config_args.push(
                    cfgr.autoLoad(emulator_start_item.textContent),
                  );
                }
                let fliplists = [];
              ...
              return Promise.all(
                get_files(cfgr, metadata, modulecfg, filelist),
              );
            },
            function () {
              if (splash.failed_loading) {
    ...
    function locateAdditionalJS(filename) {
      if (
        "file_locations" in modulecfg &&
        filename in modulecfg.file_locations
      ) {
    ...
    function get_dosbox_files(cfgr, metadata, modulecfg, filelist) {
      ...
      files_with_ext_from_filelist(filelist, meta.emulator_ext).forEach(
        function (file, i) {
          drives[default_drive] = file.name;
        },
      );
      meta_props_matching(meta, /^dosbox_drive_([a-zA-Z])$/).forEach(
        function (result) {
          let key = result[0],
            match = result[1];
      ...
      mounts.forEach(function (drive, i) {
        ...
        if (filename.toLowerCase().endsWith(".zip")) {
          ...
          files.push(
            cfgr.mountFile("/" + filename, cfgr.fetchFile(title, url)),
          );
        }
      });
      return files;
    ...
    function get_vice_files(cfgr, metadata, modulecfg, filelist) {
      ...
      files_with_ext_from_filelist(filelist, "conf").forEach(
        function (file, i) {
          wanted_files.push(file.name);
        },
      );
      meta_props_matching(meta, /^vice_drive_([89])$/).forEach(
        function (result) {
          let key = result[0],
            match = result[1];
          drives[match[1]] = meta[key];
      ...
      wanted_files.forEach(function (file, i) {
        ...
        if (filename.toLowerCase().endsWith(".zip") && false) {
          ...
          files.push(
            cfgr.mountZip(
              "", // TODO: This is a hack, no drive actually applicable here
    ...
    function get_mame_files(cfgr, metadata, modulecfg, filelist) {
      ...
      bios_files.forEach(function (fname, i) {
        if (fname) {
          var title =
            "Bios File (" + (i + 1) + " of " + bios_files.length + ")";
          files.push(
            cfgr.mountFile(
              "/" + fname,
      ...
      meta_props_matching(meta, /^mame_peripheral_([a-zA-Z0-9]+)$/).forEach(
        function (result) {
          let key = result[0],
            match = result[1];
      ...
      Object.keys(peripherals).forEach(function (periph) {
        files.push(
          cfgr.peripheral(
            periph, // we're not pushing a 'file' here,
            peripherals[periph],
          ),
        ); // but that's ok
      });
      ...
      files.push(
        cfgr.mountFile(
          "/" + modulecfg["driver"] + ".cfg",
          cfgr.fetchOptionalFile(
            "CFG File",
    ...
    function get_sae_files(cfgr, metadata, modulecfg, filelist) {
      ...
      game_files.forEach(function (file, i) {
        if (file) {
          var title =
              "Game File (" + (i + 1) + " of " + game_files.length + ")",
            url = file.name.includes("/")
              ? get_zip_url(file.name)
              : get_zip_url(file.name, get_item_name(game));
          files.push(
            cfgr.mountFile("/" + file.name, cfgr.fetchFile(title, url)),
          );
          files.push(
            cfgr.floppy(
              0, // we're not pushing a file here
              file.name,
            ),
          ); // but that's ok
        }
    ...
    function get_pce_files(cfgr, metadata, modulecfg, filelist) {
      ...
      meta_props_matching(meta, /^pce_drive_([a-zA-Z0-9]+)$/).forEach(
        function (result) {
          var key = result[0],
            periph = result[1][1];
      ...
      game_files.forEach(function (filename, i) {
        ...
        files.push(
          cfgr.mountFile(
            "/" + game_files_counter[filename] + "." + ext,
            cfgr.fetchFile(title, url),
          ),
        );
      });
    ...
    var get_files_url = function (game_path) {
      var path = game_path.split("/");
      return (
        "//cors.archive.org/cors/" + path[0] + "/" + path[0] + "_files.xml"
      );
    };
    ...
    function mountat(drive) {
      return function (data) {
        return { drive: drive, mountpoint: "/" + drive, data: data };
      };
    }
  ...
  BaseLoader.mountZip = function (drive, file) {
    return { files: [{ drive: drive, mountpoint: "/" + drive, file: file }] };
  };

  BaseLoader.mountFile = function (filename, file) {
    return { files: [{ mountpoint: filename, file: file }] };
  };
  ...
  DosBoxLoader.mountImage = function (
    drive,
    filepath,
    drive_type,
    file_system,
  ...
  function MAMELoader() {
    var config = Array.prototype.reduce.call(arguments, extend);
    config.emulator_arguments = build_mame_arguments(
      config.muted,
      config.mame_driver,
      config.nativeResolution,
      config.sample_rate,
      config.peripheral,
  ...
  function VICELoader() {
    ...
    config.emulator_arguments = build_vice_arguments(
      config.emulatorStart,
      config.files,
      config.fliplist,
      config.extra_vice_args,
    );
    config.runner = EmscriptenRunner;
    return config;
  }
  ...
  VICELoader._create_fliplist_file = function (files, fliplists) {
    ...
    files.push(
      VICELoader.mountFile(
        "/metadata_fliplist.vfl",
        VICELoader.localFile("Fliplist", fliplist),
      ).files[0],
    );
  };
  ...
  function PCELoader() {
    var config = Array.prototype.reduce.call(arguments, extend);
    config.emulator_arguments = [
      "-c",
      "/emulator/pce-" + config.pceModel + ".cfg",
    ];
    if (config.extra_pce_args && config.extra_pce_args.length > 0) {
      config.emulator_arguments = config.emulator_arguments.concat(
        config.extra_pce_args,
      );
    }
    config.runner = EmscriptenRunner;
    return config;
  ...
  var build_mame_arguments = function (
    muted,
    driver,
    native_resolution,
    sample_rate,
    ...
    if (
      native_resolution &&
      "width" in native_resolution &&
      "height" in native_resolution
    ) {
      args.push(
        "-resolution",
        [native_resolution.width, native_resolution.height].join("x"),
      );
    }
  ...
  var build_dosbox_arguments = function (
    emulator_start,
    files,
    extra_args,
    images,
    ...
    for (var i = 0; i < len; i++) {
      if ("drive" in files[i]) {
        ...
        if (files[i].drive_type === "hdd") {
          args.push(
            "-c",
            "mount " + files[i].drive + " /emulator" + files[i].mountpoint,
          );
  ...
  var build_vice_arguments = function (
    emulator_start,
    files,
    fliplist,
    extra_args,
  ) {
    var args = emulator_start
      ? ["-autostart", "/emulator/" + emulator_start]
      : [];
  ...
  function SAERunner(canvas, game_data) {
    ...
    if (
      ...
      game_data.nativeResolution.height == 360 &&
      game_data.nativeResolution.width == 284
    ) {
      ...
      game_data.nativeResolution &&
      game_data.nativeResolution.height == 1440 &&
      game_data.nativeResolution.width == 568
    ) {
      ...
      this._cfg.video.size_win.width =
        SAEC_Video_DEF_AMIGA_WIDTH << 2; /* 1440 */
      this._cfg.video.size_win.height =
        SAEC_Video_DEF_AMIGA_HEIGHT << 1; /* 568 */
    } else {
      this._cfg.video.hresolution = SAEC_Config_Video_HResolution_HiRes;
      this._cfg.video.vresolution = SAEC_Config_Video_VResolution_Double;
      this._cfg.video.size_win.width =
        SAEC_Video_DEF_AMIGA_WIDTH << 1; /* 720 */
      this._cfg.video.size_win.height =
        SAEC_Video_DEF_AMIGA_HEIGHT << 1; /* 568 */
    }

    this._cfg.memory.rom.name = game_data.rom;
    this._cfg.memory.rom.data = game_data.fs.readFileSync(
      "/" + game_data.rom,
      null,
      flag_r,
    );
    ...
    if (game_data.extRom) {
      ...
      this._cfg.memory.extRom.data = game_data.fs.readFileSync(
        "/" + game_data.extRom,
        null,
        flag_r,
      );
    ...
    for (var i = 0; i < Object.keys(game_data.floppy).length; i++) {
      ...
      this._cfg.floppy.drive[i].file.data = game_data.fs.readFileSync(
        "/" + game_data.floppy[i],
        null,
        flag_r,
      );
      this._cfg.floppy.drive[i].file.size =
        this._cfg.floppy.drive[i].file.data.length;
    }
  }
  ...
  function Emulator(canvas, callbacks, loadFiles) {
    ...
    if (
      /archive\.org$/.test(document.location.hostname) &&
      document.getElementById("gofullscreen")
    ) {
      document
    ...
    var start = function (options) {
      ...
      has_started = true;
      var defaultOptions = { waitAfterDownloading: false, hasCustomCSS: false };
    ...
    function setupSplash(canvas, splash, globalOptions) {
      ...
      if (!splash.splashElt) {
        ...
        if (!globalOptions.hasCustomCSS) {
          ...
          splash.splashElt.style.backgroundColor =
            splash.getColor("background");
        }
